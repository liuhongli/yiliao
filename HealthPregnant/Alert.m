...
- (void)addButtonsToView: (UIView *)container
{
    ...
    for (int i = 0; i < [buttonTitles count]; i++) {
        ...
        [closeButton setTitleColor:MainColor forState:UIControlStateNormal];
        [closeButton setTitleColor:MainColor forState:UIControlStateHighlighted];
...
+ (void)showWithTitle:(NSString *)title Message:(NSString *)message automaticllyDismiss:(BOOL)automatic
{
    ...
    [alert show];
    if (automatic) {
         [NSTimer scheduledTimerWithTimeInterval:1.6 target:self selector:@selector(showAlert:) userInfo:alert repeats:NO];
    }
}
